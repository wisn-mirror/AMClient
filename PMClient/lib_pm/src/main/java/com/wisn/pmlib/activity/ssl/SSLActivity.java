...
import com.wisn.pmlib.utils.TwoWaysAuthenticationSSLSocketFactory;

public class SSLActivity extends BaseActivity implements View.OnClickListener{
	private static final String HTTPS_URL = "https://10.128.230.2:8443";
	private WebView webView = null;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		this.setContentView(R.layout.activity_ssl);
		...
		AsyncTask <String, Void, String> testTask = new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... params) {
				...
				try {
					...
					conn.setSSLSocketFactory(TwoWaysAuthenticationSSLSocketFactory.getSSLSocketFactory(SSLActivity.this));
					conn.connect();
					result = parseSendMessageResponse(conn.getInputStream());
                    log.d("runHttpsRequestWithHttpsURLConnection",result);
                    return result;
	...
	private void runHttpsRequestWithHttpClient(){
		AsyncTask <String, Void, String> testTask = new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... params) {
				try {
					HttpClient httpsClient = MApplication.getInstance().getHttpsClient(SSLActivity.this.getBaseContext());
